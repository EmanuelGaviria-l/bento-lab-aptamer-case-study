#!/usr/bin/env bash
# Run AlphaFold 3 on inputs/af3_msa/ (cached protein MSAs inlined in JSON).
#
# Does NOT depend on which venv is currently active — uses AF3's own python.
#
# Usage (from anywhere, on a GPU node):
#   nohup bash scripts/af3/run_all_msa.sh > docs/af3_msa_batch_log.txt 2>&1 &

set -uo pipefail

PROJECT_ROOT="/projects/bentosprg6/gavirial/bento-lab-aptamer-case-study"
INPUT_DIR="$PROJECT_ROOT/inputs/af3_msa"
OUTPUT_DIR="$PROJECT_ROOT/results/af3_msa"
